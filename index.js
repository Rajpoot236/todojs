...
function renderTodo(todo) {
  ...
  for (let i = 0; i < todoItems.length; i++) {
    ...
    for (let j = 0; j < currentTodo.subTask.length; j++) {
      ...
      let rest = currentTodo.subTask[i].marked ? "" : `<input class="markDone" type="checkbox" onclick="markCompleted(this)">`;
      const liNode = document.createElement("li");
      liNode.setAttribute("class", classToPut);
      liNode.setAttribute("data-key", currentTodo.subTask[j].id);
      liNode.innerHTML = `${rest} ${currentTodo.subTask[j].name}`;
...
function markCompleted(element) {
  let classToPut = flag ? "card-item card-item-checked" : "card-item-2 card-item-checked";
...
function addSubTodo() {
  ...
  if (taskHeading !== "") {
    ...
    node.innerHTML = `<input class="markDone" type="checkbox" onclick="markCompleted(this)"> ${taskHeading}`;
    let currentTodo;
    //Find in the todo array
    for (let i = 0; i < todoItems.length; i++) {
      if (todoItems[i].id == id) {
...
function redirect(element) {
  ...
  for (let i = 0; i < currentTodo.subTask.length; i++) {
    ...
    node.innerHTML = `${rest} ${currentTodo.subTask[i].name}`;
    e.append(node);
  }
}
function goBack() {
